class Solution {
    public int numDecodings(String s) {
        // we check if the string doesn't start with 0 as we would return 0
        if(s.charAt(0) == '0') return 0;

        // we now set our two base cases
        // first two values to build up from

        // when the string is empty (we are guarenteed non empty string but when we reach the end), we return 1
        // we know the first character is not 0, as per the first check, so we have 1 way to decode 1 digit
        // p2 is dp[i - 2] 
        // p1 is dp[i - 1]
        int p1 = 1, p2 = 1;
        ...
        for(int i = 1; i < s.length(); i++){
            ...
            // we ensure that the first character was not a leading 0
            // this is for one digit decode
            if (s.charAt(i) != '0') temp += p1;

            // we now do 2 digit decode
            // we check if the two digit is in range of 10 and 26
            int twoDigit = Integer.parseInt(s.substring(i - 1, i + 1));
            if (twoDigit >= 10 && twoDigit <= 26) temp += p2;
