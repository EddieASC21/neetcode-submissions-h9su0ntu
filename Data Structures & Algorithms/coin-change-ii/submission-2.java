class Solution {
    public int change(int amount, int[] coins) {
        ...
        // there is 1 way to sum up to 0
        dp[0] = 1;

        // we go through all the coins
        for(int i = coins.length - 1; i > -1; i--){
            // we create the new dp array for next coin
            int[] newDp = new int[amount + 1];
            // we set the same base case
            newDp[0] = 1;

            // we iterate through the amounts noting that amount 0 is set to 1
            for(int a = 1; a <= amount; a++){
                // we skip the current coin
                newDp[a] = dp[a];
                // if we want to use this coin we check if we are not outbounds and update it
                if(a - coins[i] >= 0) newDp[a] += newDp[a - coins[i]];
            }

            // set our new dp array for next coin
            dp = newDp;
        }
