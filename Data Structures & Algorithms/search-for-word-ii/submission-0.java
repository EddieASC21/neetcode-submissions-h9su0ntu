...
class Solution {
    ...
    private void search(char[][] board, int row, int column, TrieNode root, String word, Set<String> visited, Set<String> result){
        String position = row + "," + column;
        
        if(row < 0 || row >= board.length || column < 0 || column >= board[0].length || visited.contains(position)) return;
