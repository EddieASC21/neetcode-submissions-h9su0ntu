class Solution {
    public int maxAreaOfIsland(int[][] grid) {
        ...
        // we use a hash set to keep track of the visited cells
        ...
        for(int i = 0; i < grid.length; i++){
            for(int j = 0; j < grid[0].length; j++){
                ...
                maxArea = Math.max(maxArea, helper(grid, visited, i, j));
            }
        }
    ...
    private int helper(int[][] grid, Set<String> visited, int i, int j){
        // base case

        // check if we go out of bounds
        // we also check if we are on water
        // we also check if we already visited the cell 
        // so we return 0 as we did not find an island
        if(i < 0 || i == grid.length || j < 0 || j == grid[0].length || grid[i][j] == 0 || visited.contains(i + "," + j)) return 0;

        // if not caught by base case
        // we are on an island
        // we mark its position as visited and add it to the set
        visited.add(i + "," + j);
